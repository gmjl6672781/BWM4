...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Teleport : MonoBehaviour
    {
        ...
        public event Action<bool> OnTeleport;
        //-------------------------------------------------
        private static Teleport _instance;
        public float deltDis = 0.05f;
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.UpArrow))
            {
                ...
                player.trackingOriginTransform.position += Vector3.up * deltDis;
            }
            if (Input.GetKeyDown(KeyCode.DownArrow))
            {
                //sysOffset -= Vector3.up * deltDis;
                player.trackingOriginTransform.position -= Vector3.up * deltDis;
        ...
        private void TeleportPlayer()
        {
            ...
            {
                Vector3 playerFeetOffset = player.trackingOriginTransform.position - player.feetPositionGuess;
                Vector3 lastPos = player.trackingOriginTransform.position;
                Vector3 tmp = teleportPosition + playerFeetOffset;
                tmp.y = lastPos.y;
                player.trackingOriginTransform.position = tmp;
        ...
        ///位移提示
        /////-------------------------------------------------
        public void ShowTeleportHint(string message)
        {
            CancelTeleportHint();

            hintCoroutine = StartCoroutine(TeleportHintCoroutine(message));
        }

        //-------------------------------------------------
        private IEnumerator TeleportHintCoroutine(string message)
        {
            float prevBreakTime = Time.time;
            float prevHapticPulseTime = Time.time;
